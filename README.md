...
- **Articles:** A news item with `inline: false` and its own `permalink: /publications/<slug>/` is a full article about a paper, with a 1200x630 card built from `assets/cards/<key>.html` by `scripts/build-card.sh`. The `paper-article` skill (`.claude/skills/paper-article/`) holds the workflow and the prose style; `_news/2025-10-11-beer-fermentation-digital-twin.md` and `_news/2026-07-16-iswc-2026-oml-paper.md` are the worked examples.
- **Abstracts in articles:** `{% include abstract.liquid text="..." %}` renders the paper's abstract collapsed behind an "Abstract" toggle; the text stays in the HTML, so it is still indexed. Use single quotes inside the text.
- **Article figures:** Plain `<figure><img>` blocks, never wider than the column. Set the width with a class on the `<figure>` (defined in `_sass/_blog.scss`): `figure-full` for diagrams whose text needs the full column, `figure-narrow` for photos (80%, full width on phones). Avoid an inline `width` on the image, since it overrides the class.
